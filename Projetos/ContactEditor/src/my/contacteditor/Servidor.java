...
//        nomesUsuarios=nomesUsuarios1;
        ...
        System.out.println("Servidor em execucao. porta: " + porta);
        while (true) {
    ...
    public void run() {
        try {
            ...
            if (!bWhile) {
                String msg = "ok";
                oos.writeObject(msg);
                System.out.println("Usuario " + usuarioRecebido + " conectou."); //mostra qual usuario ntrou no chat
                InetAddress ipCliente = conexao.getInetAddress(); //pega ip do usuario
                int portaCliente = conexao.getPort(); //pega porta do usuario
                ...
                disponiveis.add(pegaUsuario);//adiciona usuario que acabou de entrar na lista de disponiveis?
    ...
    public static void atualizaLista(ObjectOutputStream oos, Usuario pegaUsuario, ArrayList<String> auxDisponiveis, ArrayList<String> auxIndisponiveis) throws IOException {
        ...
        while (iteratorUsuario.hasNext()) {
            ...
            for (int i = 0; i < possivelAmigo.amigos.length; i++) {
                System.out.println(nomeCompleto + " " + possivelAmigo.amigos[i]);
